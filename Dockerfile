...
RUN wget -O /tmp/maven.tar.gz http://archive.apache.org/dist/maven/maven-3/$DOCKERIMAGE_MAVEN_VERSION/binaries/apache-maven-$DOCKERIMAGE_MAVEN_VERSION-bin.tar.gz && mkdir -p $M3_HOME && tar --strip 1 -zxvf /tmp/maven.tar.gz -C $M3_HOME
RUN go get -u github.com/tcnksm/ghr
RUN echo -n "#!/bin/bash\nSRC=\$(dirname \$0)\njava -jar \$SRC/$DOCKERIMAGE_SAXON_JAR \$@\n" > /usr/local/bin/saxon
RUN chmod a+x /usr/local/bin/saxon

ENV PATH $GOPATH/bin:$JAVA_HOME/bin:$M3_HOME/bin:$PATH
